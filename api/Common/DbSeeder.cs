...
public static class DbSeeder
{
    ...
    public static async Task SeedAsync(AppDbContext db, IConfiguration? config = null)
    {
        ...
        if (forceReset)
        {
            ...
            Console.WriteLine("[DbSeeder] Tài khoản: 0900000000..0007 / MK: 123456");
    ...
    private static async Task SeedFreshAsync(AppDbContext db, IConfiguration? config)
    {
        ...
        // ─── 1) Users ──────────────────────────────────────────────────────
        var admin = NewUser("0900000000", "Admin Trường", UserRole.Admin, hash);
        var teacher = NewUser("0900000002", "Nguyễn Văn A (GV)", UserRole.Teacher, hash);
        var teacher2 = NewUser("0900000006", "Trần Thị B (GV)", UserRole.Teacher, hash);
        var parent = NewUser("0900000003", "Lê Minh Parent", UserRole.Parent, hash);
        var studentAn = NewUser("0900000004", "Nguyễn Văn An", UserRole.Student, hash);
        var studentBinh = NewUser("0900000005", "Nguyễn Thị Bình", UserRole.Student, hash);
        var studentCuong = NewUser("0900000007", "Phạm Văn Cường", UserRole.Student, hash);

        db.Users.AddRange(admin, teacher, teacher2, parent, studentAn, studentBinh, studentCuong);
        ...
        {
            var cls = new Class { Name = "10A1", SemesterId = sem.Id };
            db.Classes.Add(cls);
            await db.SaveChangesAsync();
            ...
            // GV A dạy mọi môn; GV B dạy thêm Anh (đủ show nhiều GV).
            ...
            AddTimetable(db, cls, teacher, math, lit, eng, phy, che);
            await db.SaveChangesAsync();
        }
    ...
    private static User NewUser(string phone, string fullName, UserRole role, string passwordHash)
        => new()
        {
            Phone = phone,
            Username = phone,
    ...
    /// <summary>Thêm TKB tuần mẫu cho 1 lớp (T2–T6, 3 tiết/ngày).</summary>
    ...
    /// <summary>
    /// Điểm form THPT đã Publish: Miệng / 15p / 1 tiết / GK / CK cho 4 môn.
    ...
    {
        var plan = new (Subject Subject, string Type, double Score)[]
        {
            (math, "Oral", 8.5), (math, "Quiz15", 7.5), (math, "OnePeriod", 8.0),
            (math, "Midterm", 7.5), (math, "Final", 8.0),
            (lit, "Oral", 8.0), (lit, "Quiz15", 7.0), (lit, "OnePeriod", 7.5),
            (lit, "Midterm", 8.0), (lit, "Final", 7.5),
            (eng, "Oral", 9.0), (eng, "Quiz15", 8.5), (eng, "OnePeriod", 8.0),
            (eng, "Midterm", 8.5), (eng, "Final", 9.0),
            (phy, "Oral", 5.0), (phy, "Quiz15", 4.5), (phy, "OnePeriod", 4.0),
            (phy, "Midterm", 4.0), (phy, "Final", 3.0),
        };

        foreach (var st in students)
        {
            // Biến thiên nhẹ theo HS để bảng điểm không giống hệt nhau.
            var bias = st.FullName.Contains("Bình") ? -0.5
                : st.FullName.Contains("Cường") ? 0.3
                : 0.0;

            foreach (var (subj, type, score) in plan)
            {
                var s = Math.Clamp(score + bias, 0, 10);
