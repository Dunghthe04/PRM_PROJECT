...
public interface ITeacherAssignmentRepository
{
    ...
    /// <summary>GV có được phân công dạy lớp này không (bộ môn).</summary>
    Task<bool> TeacherAssignedToClassAsync(int teacherId, int classId);

    /// <summary>GV có phải chủ nhiệm lớp không.</summary>
    Task<bool> IsHomeroomTeacherAsync(int teacherId, int classId);
...
public class TeacherAssignmentRepository : ITeacherAssignmentRepository
{
    ...
    /// <summary>True nếu GV là chủ nhiệm lớp.</summary>
    public Task<bool> IsHomeroomTeacherAsync(int teacherId, int classId)
