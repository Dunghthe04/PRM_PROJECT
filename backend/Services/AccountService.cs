...
public class AccountService : IAccountService
{
    ...
    public async Task<(UserDto? Profile, string? Error)> UpdateProfileAsync(int userId, UpdateProfileDto dto)
    {
        ...
        // Email dùng nhận OTP quên MK — không cho trùng user khác.
        string? newEmail = string.IsNullOrWhiteSpace(dto.Email) ? null : dto.Email.Trim();
        if (newEmail != null)
        {
            var owner = await _userRepository.GetUserByEmailAsync(newEmail);
            if (owner != null && owner.Id != userId)
                return (null, "Email đã được tài khoản khác sử dụng.");
        }
